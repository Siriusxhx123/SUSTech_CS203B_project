...
//        widthField.setText(String.valueOf(this.displayImage.getWidth()));
//        heightField.setText(String.valueOf(this.displayImage.getHeight()));
        ...
        originalSizeLabel =  new JLabel("原始尺寸: - x -");
        originalSizeLabel.setFont(originalSizeLabel.getFont().deriveFont(Font.PLAIN));
        inputPanel.add(new JLabel("宽度:"));
        inputPanel.add(widthField);
        inputPanel.add(new JLabel("高度:"));
        inputPanel.add(heightField);
//        inputPanel.add(originalSizeLabel);
...
//        JButton resizeButton = new JButton("resizeButton");
...
//        updateOriginalSizeLabel();
        ...
        saveButton.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    BufferedImage resizedImage = resizeImage(bufferedImage, newWidth, newHeight, frame);
                    // 选择保存路径
                    JFileChooser saveChooser = new JFileChooser();
                    saveChooser.setFileSelectionMode(JFileChooser.DIRECTORIES_ONLY);
                    int saveRetVal = saveChooser.showSaveDialog(null);
                    if (saveRetVal == JFileChooser.APPROVE_OPTION) {
                        File saveDirectory = saveChooser.getSelectedFile();
                        String randomFileName = generateRandomFileName() + ".jpg";
                        File outputFile = new File(saveDirectory, randomFileName);
                        ImageIO.write(resizedImage, "jpg", outputFile);
                        JOptionPane.showMessageDialog(null, "图片已保存至：" + outputFile.getAbsolutePath(), "成功", JOptionPane.INFORMATION_MESSAGE);
                    }
                } catch (NumberFormatException ex) {
                    JOptionPane.showMessageDialog(null, "请输入有效的宽度和高度。", "错误", JOptionPane.ERROR_MESSAGE);
                } catch (IOException ex) {
                    JOptionPane.showMessageDialog(null, "保存图片失败", "错误", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
    ...
    private String generateRandomFileName() {
        ...
        return String.format("%08d", rand.nextInt(Integer.MAX_VALUE)); // 生成8位数字的随机文件名
    }
    ...
    private void previewResizedImage(JFrame frame) {
        try {
            int newWidth = Integer.parseInt(widthField.getText());
            int newHeight = Integer.parseInt(heightField.getText());
            BufferedImage resizedImage = resizeImage(bufferedImage, newWidth, newHeight, frame);
    ...
    private BufferedImage resizeImage(BufferedImage original, int newWidth, int newHeight, JFrame frame) {
        Image scaledImage = original.getScaledInstance(newWidth, newHeight, Image.SCALE_SMOOTH);
        BufferedImage resizedImage = new BufferedImage(newWidth, newHeight, BufferedImage.TYPE_INT_RGB);
        Graphics2D g2d = resizedImage.createGraphics();
        g2d.drawImage(scaledImage, 0, 0, null);
        g2d.dispose();
        frame.setTitle("Karve - " + newWidth + " x " + newHeight);
        return resizedImage;
    }
    ...
    private void updateOriginalSizeLabel() {
        originalSizeLabel.setText("原始尺寸: " + this.displayImage.getWidth() + " x " + this.displayImage.getHeight());
    }
